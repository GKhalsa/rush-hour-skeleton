...
class UAgentTest < Minitest::Test
  ...
  def test_it_knows_its_user_browser_and_os
    user_agent = UAgent.create({:browser => "Mozilla/5.0",
                                   :os => "Macintosh"})
  ...
  def test_is_invalid_with_missing_user_agent
    user_agent = UAgent.create(browser: "")
    refute user_agent.valid?
  end

  def test_it_can_provide_browser_breakdown
    create_payloads(3)
    user_agent1 = UAgent.create(browser: "Chrome", os: "Macintosh")
    user_agent2 = UAgent.create(browser: "Mozilla", os: "Windows")
    ...
    assert_equal expected, UAgent.browser_breakdown
  end

  def test_it_can_provide_an_os_breakdown
    create_payloads(3)
    user_agent1 = UAgent.create(browser: "Chrome", os: "Windows")
    user_agent2 = UAgent.create(browser: "Mozilla", os: "Macintosh")
    PayloadRequest.all[0].update(user_agent_id: user_agent1.id)
    PayloadRequest.all[1].update(user_agent_id: user_agent2.id)
